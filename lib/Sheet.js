...
class Sheet {
	...
	*//**
	 ...
	 * @param {string} name - defined name for the image.
   ...
   * * @param {Blob} file - the image.
	 * @returns {Drawing} Drawing Class for that image.
	 * @ignore
	 */
    ...
    drawings() {
        return new ArgHandler('Sheet.drawings')
            .case(() => {
              return Object.values(this._drawings);
            })
            .case('string', name => {
                return _.find(this._drawings, ['_name', name]);
            })
            .case(['string', 'blob'], (name, file) => {
                // TODO implement for svg images                 
                this._drawingId = 0
                let mediaId = 0;
                _.forEach(this._workbook._sheets, (sheet) => {
                    let drawingCount = sheet.drawings().length;
                    if (drawingCount > 0) {
                      this._drawingId++;
                        mediaId = mediaId + drawingCount;
                    }
                });
                
                const index = Object.keys(this._drawings).length;
                const id = `rId${index + 1}`;
                const path = `../media/image${mediaId + 1}.png`;

                const relationship = this._drawingRelationships.add("image", path);
                const svgRelationship = undefined; // TODO
        ...
        if (Object.keys(this._drawings).length > 0) {
            ...
            if (!relationship) {
                relationship = this._relationships.add(
                    "drawing",
                    `../drawings/drawing${this._drawingId + 1}.xml`
                );
            }
            ...
            this._drawingNodes.forEach((drawingNode, i) => {
                if (drawingNode && drawingsReplationshipNode) {
                    if (drawingNode.name.includes('twoCellAnchor')) {
                        const imageId = drawingNode.children[2].children[1].children[0].attributes['r:embed'];
                        const imageRelationship = this._drawingRelationships.findById(imageId);                        
                        if (imageRelationship) {
                            // Excel stores svg images as uncompressed png and svg
                            const svgId = drawingNode.children[2].children[1].children[0].children[0].children[0].children[0].attributes['r:embed'];
                            const svgRelationship = this._drawingRelationships.findById(svgId);
                            this._drawings[i] = new Drawing(i, workbook, this, drawingNode, imageRelationship, svgRelationship);
                        }
                    } else {
