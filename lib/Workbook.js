...
class Workbook {
    ...
    outputAsync(opts) {
        ...
        this._sheets.forEach((sheet, i) => {
            ...
            if (Object.keys(sheet._drawings).length > 0) {
              ...
              // Use the drawing relationship to establish the index for the drawing, if it does not exist then default to sheet index
              const drawingRelationship = sheet._relationships.findByType('drawing');
              const idx = drawingRelationship ? drawingRelationship.attributes.Target.replace('../drawings/drawing', '').replace('.xml', '') : i + 1;
            
              const drawingPath = `xl/drawings/drawing${idx}.xml`;
              const drawingRelsPath = `xl/drawings/_rels/drawing${idx}.xml.rels`;
              ...
              _.each(sheet._drawings, drawing => {
                ...
                sheet._drawingRelationships[sheet._drawingRelationships.findById(drawing._svgId)] = xml[2];
              });
        ...
        this._zip.file("_rels/.rels", xmlBuilder.build(this._docRelationships), zipFileOpts);
        ...
        this._zip.file("docProps/custom.xml", xmlBuilder.build(this._customProperties), zipFileOpts);
     ...
     */
    clearProperties() {
        return this._coreProperties.clear();
    }
     ...
     * Gets multiple properties.
     * @param {Array.<string>} names - The names of the properties.
     * @returns {object.<string, *>} Object whose keys are the custom property names and values are the properties.
     *//**
     ...
     * @param {object.<string, *>} custom properties - Object whose keys are the property names and values are the values to set.
     * @returns {Workbook} The workbook.
     */
    customProperty() {
        return new ArgHandler("Workbook.customProperty")
            .case('string', name => {
                // Get single value
                return this._customProperties.get(name);
            })
            .case('array', names => {
                // Get list of values
                const values = {};
                names.forEach(name => {
                    values[name] = this._customProperties.get(name);
                });
            ...
            .case(['string', '*'], (name, value) => {
                // Set a single value for all cells to a single value
                this._customProperties.set(name, value);
                return this;
            })
            .case('object', nameValues => {
                // Object of key value pairs to set
     ...
     * @returns {CustomProperties} The custom properties.
     */
    customProperties() {
    ...
    clearCustomProperties() {
        return this._customProperties.clear();
    }
    ...
    _initAsync(data, opts) {
        ...
        return externals.Promise.resolve()
            ...
            .then(nodes => {
                ...
                const docRelationshipsNode = nodes[1];
                const appPropertiesNode = nodes[2];
                const corePropertiesNode = nodes[3];
                const customPropertiesNode = nodes[4];
                const relationshipsNode = nodes[5];
                const sharedStringsNode = nodes[6];
                const styleSheetNode = nodes[7];
                const workbookNode = nodes[8];

                // Load the various components.
                this._contentTypes = new ContentTypes(contentTypesNode);
                this._docRelationships = new Relationships(docRelationshipsNode);
                this._appProperties = new AppProperties(appPropertiesNode);
                this._coreProperties = new CoreProperties(corePropertiesNode);
                this._customProperties = new CustomProperties(customPropertiesNode);
                ...
                // Add the custom properties relationship if it doesn't exist.                
                if (!this._docRelationships.findByType("custom-properties")) {
                    this._docRelationships.add("custom-properties", "docProps/custom.xml");
                }
                
                // Add the custom properties content type if it doesn't exist.
                if (!this._contentTypes.findByPartName("/docProps/custom.xml")) {
                    this._contentTypes.add("/docProps/custom.xml", "application/vnd.openxmlformats-officedocument.custom-properties+xml");
                }
                ...
                return externals.Promise.all(_.map(this._sheetsNode.children, (sheetIdNode, i) => {
                    ...
                    return this._parseNodesAsync([`xl/worksheets/sheet${i + 1}.xml`, `xl/worksheets/_rels/sheet${i + 1}.xml.rels`])
                        .then(nodes => {
                            ...
                            const drawingRelationship = (new Relationships(sheetRelationshipsNode)).findByType("drawing");

                            // Load any drawings that belong to this sheet.
                            if (drawingRelationship) {
                              const idx = drawingRelationship.attributes.Target.replace('../drawings/drawing', '').replace('.xml', '');

                              return this._parseNodesAsync([`xl/drawings/drawing${idx}.xml`, `xl/drawings/_rels/drawing${idx}.xml.rels`])          
                                .then(nodes => {
                                  const drawingsNode = nodes[0];
                                  const drawingsRelationshipsNode = nodes[1];
